    ...
    <Container>
      ...
      <form onSubmit={submitHandler}>
        <input
          ref={inputRef}
          value={url}
          onChange={() => setUrl(inputRef.current.value)}
        />
      ...
      <Img
        ...
        src={`https://icon.horse/icon/${iconUrl}`}
        alt={iconUrl}
      ></Img>
    </Container>
  );
...
const queryToData = ({ query }) => {
  if (
    !isTriggered(query, ["icon", "favicon", "icons", "favicons"], {
      substringMatch: true,
    })
  ) {
  ...
  if (words.includes("website") || words.includes("site"))
