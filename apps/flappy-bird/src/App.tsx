...
const gameLogic = () => {
  const can = document.getElementById("stage") as HTMLCanvasElement;
  if (!can) return;
  ...
  const init = () => {
    ...
    pPat = ctx.createPattern(createPattern() as HTMLCanvasElement, "repeat");
    ...
    gPat = ctx.createPattern(createPattern2() as HTMLCanvasElement, "repeat");
  };
  ...
  const adjust = () => {
    ...
    for (const i in pillars) {
      ...
      if (
        ...
        if (!pill.passed && Number.parseInt(pillIndex) % 2 === 1) {
          score += 1;
          pill.passed = true;
        }
      }
